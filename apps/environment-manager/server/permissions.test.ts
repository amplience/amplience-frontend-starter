...
/** Build a FetchJson stub from a URL-prefix → response table (first match wins). */
function stubFetch(routes: [string, { status: number; body?: unknown }][]): FetchJson {
  return (url) => {
...
function happyRoutes(): [string, { status: number; body?: unknown }][] {
  return [
...
describe('buildPermissionsReport', () => {
  it('reports full capability on the happy path', async () => {
    ...
    for (const key of ['settings', 'schemas', 'types', 'extensions']) {
  ...
  it('marks hub-level writes denied and names the missing links', async () => {
    ...
    routes[4] = [
      `${API}/hubs/hub123`,
      { status: 200, body: { _links: { 'create-content-type-schema': { href: 'x' } } } },
    ]
  ...
  it('treats a hub body without _links as unknown writes', async () => {
    const routes = happyRoutes()
    routes[4] = [`${API}/hubs/hub123`, { status: 200, body: {} }]
    const report = await buildPermissionsReport(ENV, stubFetch(routes))
  ...
  it('marks denied read probes with their status', async () => {
    const routes = happyRoutes()
    routes[1] = [`${API}/hubs/hub123/content-type-schemas`, { status: 403 }]
    const report = await buildPermissionsReport(ENV, stubFetch(routes))
  ...
  it('flags a 404 repo as a likely-wrong ID', async () => {
    const routes = happyRoutes()
    routes[5] = [`${API}/content-repositories/repoC`, { status: 404 }]
    const report = await buildPermissionsReport(ENV, stubFetch(routes))
  ...
  it('marks a readable repo without create-content-item as write-denied', async () => {
    const routes = happyRoutes()
    routes[6] = [`${API}/content-repositories/repoS`, { status: 200, body: { _links: {} } }]
    const report = await buildPermissionsReport(ENV, stubFetch(routes))
