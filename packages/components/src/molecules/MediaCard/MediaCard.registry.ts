 ...
 * `localeBasePath` and `sizes` are excluded: both are renderer-injected from
 * context (locale and parent column geometry), not author-set.
 */
export type MediaCardSchema = Omit<MediaCardProps, 'localeBasePath' | 'sizes'> & {
 ...
 * stays inside the active locale, and forwards the parent's `slotSizes` (when
 * present) so the cover image sizes its srcset to the real slot; the remaining
 * fields are the component's props one-for-one.
 */
